...
class CatView
{
	...
	private function categoriePrest(){
		...
		if ($this->order == "desc") {
			$prestations = new PrestaView($this->app, [$categorie->prestations()->sortByDesc('prix'), $this->order]);
		} else {
			$prestations = new PrestaView($this->app, [$categorie->prestations()->sortBy('prix'), $this->order]);
		}
